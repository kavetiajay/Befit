-- =====================================================================
-- BEFIT GYM MANAGEMENT SYSTEM DATABASE SCHEMA MIGRATION (FINAL)
-- STEP 4 - DATABASE TABLE EXECUTION (SECURITY HARDENED & SYNTAX VALIDATED)
-- =====================================================================
...
-- Trigger function to enforce read-only properties on notifications updates
-- Enforces that clients can only toggle is_read and update read_at fields.
CREATE OR REPLACE FUNCTION public.prevent_notification_fields_change()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.id IS DISTINCT FROM OLD.id OR
       ...
       NEW.created_at IS DISTINCT FROM OLD.created_at THEN
        RAISE EXCEPTION 'Only the is_read and read_at columns may be updated on notifications.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_catalog;
...
-- Bind the update prevention trigger to public.notifications
CREATE TRIGGER check_notifications_update BEFORE UPDATE ON public.notifications FOR EACH ROW EXECUTE FUNCTION public.prevent_notification_fields_change();
...
-- =====================================================================
-- 6. RLS POLICIES DEFINITION (Explicitly structured)
-- =====================================================================
...
CREATE POLICY "profiles_select_assigned_clients" ON public.profiles FOR SELECT TO authenticated USING (public.get_user_role(auth.uid()) = 'trainer' AND role = 'client' AND public.is_client_assigned_to_trainer(id, auth.uid()));
...
CREATE POLICY "profiles_update_clients_by_trainer" ON public.profiles FOR UPDATE TO authenticated USING (public.get_user_role(auth.uid()) = 'trainer' AND role = 'client' AND public.is_client_assigned_to_trainer(id, auth.uid())) WITH CHECK (public.get_user_role(auth.uid()) = 'trainer' AND role = 'client' AND public.is_client_assigned_to_trainer(id, auth.uid()));
...
CREATE POLICY "exercises_insert_trainer" ON public.exercises FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) = 'trainer');
CREATE POLICY "exercises_update_trainer" ON public.exercises FOR UPDATE TO authenticated USING (public.get_user_role(auth.uid()) = 'trainer') WITH CHECK (public.get_user_role(auth.uid()) = 'trainer');
CREATE POLICY "exercises_delete_trainer" ON public.exercises FOR DELETE TO authenticated USING (public.get_user_role(auth.uid()) = 'trainer');
...
CREATE POLICY "workout_schedules_insert_trainer" ON public.workout_schedules FOR INSERT TO authenticated WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.workout_plans
...
CREATE POLICY "workout_schedules_update_trainer" ON public.workout_schedules FOR UPDATE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM public.workout_plans
    WHERE workout_plans.id = workout_schedules.workout_plan_id
    AND workout_plans.trainer_id = auth.uid()
...
CREATE POLICY "workout_schedules_delete_trainer" ON public.workout_schedules FOR DELETE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM public.workout_plans
    WHERE workout_plans.id = workout_schedules.workout_plan_id
    AND workout_plans.trainer_id = auth.uid()
...
CREATE POLICY "diet_meals_insert_trainer" ON public.diet_meals FOR INSERT TO authenticated WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.diet_plans
    WHERE diet_plans.id = diet_plan_id
    AND diet_plans.trainer_id = auth.uid()
  )
);
CREATE POLICY "diet_meals_update_trainer" ON public.diet_meals FOR UPDATE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM public.diet_plans
    WHERE diet_plans.id = diet_meals.diet_plan_id
    AND diet_plans.trainer_id = auth.uid()
...
CREATE POLICY "diet_meals_delete_trainer" ON public.diet_meals FOR DELETE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM public.diet_plans
    WHERE diet_plans.id = diet_meals.diet_plan_id
    AND diet_plans.trainer_id = auth.uid()
...
CREATE POLICY "weight_progress_insert_client" ON public.weight_progress FOR INSERT TO authenticated WITH CHECK (client_id = auth.uid());
CREATE POLICY "weight_progress_update_client" ON public.weight_progress FOR UPDATE TO authenticated USING (client_id = auth.uid()) WITH CHECK (client_id = auth.uid());
CREATE POLICY "weight_progress_delete_client" ON public.weight_progress FOR DELETE TO authenticated USING (client_id = auth.uid());
...
CREATE POLICY "weight_progress_insert_trainer" ON public.weight_progress FOR INSERT TO authenticated WITH CHECK (public.is_client_assigned_to_trainer(client_id, auth.uid()));
CREATE POLICY "weight_progress_update_trainer" ON public.weight_progress FOR UPDATE TO authenticated USING (public.is_client_assigned_to_trainer(client_id, auth.uid())) WITH CHECK (public.is_client_assigned_to_trainer(client_id, auth.uid()));
CREATE POLICY "weight_progress_delete_trainer" ON public.weight_progress FOR DELETE TO authenticated USING (public.is_client_assigned_to_trainer(client_id, auth.uid()));
...
CREATE POLICY "payments_insert_trainer" ON public.payments FOR INSERT TO authenticated WITH CHECK (public.is_client_assigned_to_trainer(client_id, auth.uid()));
CREATE POLICY "payments_update_trainer" ON public.payments FOR UPDATE TO authenticated USING (public.is_client_assigned_to_trainer(client_id, auth.uid())) WITH CHECK (public.is_client_assigned_to_trainer(client_id, auth.uid()));
CREATE POLICY "payments_delete_trainer" ON public.payments FOR DELETE TO authenticated USING (public.is_client_assigned_to_trainer(client_id, auth.uid()));
